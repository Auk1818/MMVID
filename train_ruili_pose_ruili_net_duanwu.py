...
matname = 'C:/Users/lenovo/Desktop/mission/sensys/rgb_mmwave_ruili/loss/midmodal_allfusion_38_divded'

def save_models(epoch):
    path = "C:/Users/lenovo/Desktop/mission/sensys/rgb_mmwave_ruili/log/final_model_38"
    import os
...
for epoch in range(num_epochs):
    ...
    if iftrain==1:
        ...
        for batch_idx,data in tqdm(enumerate(train_loader)):
            ...
            data_ti = torch.tensor(data_ti_in, dtype=torch.float32, device=device).squeeze()#data[1]：正样本雷达数据
            ...
            data_ti2 = torch.tensor(data_ti_in2, dtype=torch.float32, device=device).squeeze()#data[2]：负样本雷达数据

            batch_size, seq_len, dim, x, y = data[0].shape
            data_rgb_in = np.reshape(data[0], (batch_size * seq_len, dim, x, y))
            data_rgb = torch.tensor(data_rgb_in, dtype=torch.float32, device=device)#data[0]：锚点样本RGB数据

            batch_size, seq_len, dim, x, y = data[9].shape
            data_rgb_in2 = np.reshape(data[9], (batch_size * seq_len, dim,x,y))
            data_rgb2 = torch.tensor(data_rgb_in2, dtype=torch.float32, device=device)#data[9]：负样本RGB数据

            batch_size, seq_len, point_num, dim = data[6].shape
            data_key_in = np.reshape(data[6], (batch_size * seq_len, point_num, dim))
            data_key = torch.tensor(data_key_in, dtype=torch.float32, device=device)#data[6]：锚点样本的key数据

            data_key_in2 = np.reshape(data[7], (batch_size * seq_len, point_num, dim))
            data_key2 = torch.tensor(data_key_in2, dtype=torch.float32, device=device)#data[7]：正样本key数据

            data_key_in3 = np.reshape(data[8], (batch_size * seq_len, point_num, dim))
            data_key3 = torch.tensor(data_key_in3, dtype=torch.float32, device=device)#data[8]：负样本key数据
            ...
            rgb_l, ti_l, ti_l2, rgb_l_dif,ti_dif, key_pre_rgb, key_pre_ti, key_pre_ti2, key_pre_rgb_self,key_pre_ti_self,key_pre_ti_self_n, g_loc_p2, g_loc_n2,ti_dif_n,rgb_l_n2 = \
            ...
            a = 0.5
            loss = a * loss_keypoint + a*loss_loc +loss_metric_l_dif1+ a * loss_metric_l
